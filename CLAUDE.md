...
3. **`HomeworkBuilder` and `ActivityBuilder` hold their row id in state and are unkeyed** (F64) — so a change of `:id` in the URL leaves the editor bound to the previous row. `QuizBuilder` had the same shape and was keyed on 2026-08-04; these two were left alone. `PresentationBuilder` derives its id from the prop and is fine
...
- **A failed fetch must become a state, not an absence.** F63: `PresentationBuilder`'s catch cleared `loading` — textbook-looking error handling — but left `row` null, and the guard read `loading || !deck`, so the loader rendered forever. F64 was the same mistake wearing the opposite mask: `QuizBuilder` swallowed both loads in `.catch(() => {})` and rendered a blank *editable* form for a quiz that wasn't there. Both now carry a `loadError` that separates 404 from transport failure. `HomeworkBuilder` and `ActivityBuilder` have never been checked.
