...
**Day 6 is mostly done (2026-08-04).** Every teacher-facing view now reads from the dictionary — Dashboard, TeachingRail, StudioCard, Schedule + popup, all of My students, Library, Reports, Settings, Notifications and the builder error cards. EN and AR hold **1,399 keys each with exact parity**, verified signed-in in Arabic across Dashboard, Schedule, My students, Reports and Settings.

**What is still English, and why it isn't in the count:** the four builders' own field labels, the Quizzes / Homework / Presentations / Activities lists, and Studio. They never appeared in the "untranslated views" list because they already import `useT` — they are *partially* translated, which the grep could not see. That is the remaining slice of F34. Role consoles are deliberately English (`AccountDrawer` included — it is superadmin-only).

**Stored enum values still render English in Arabic** — grades, sections, emirates, curricula, nationalities, majors. They are CHECK-constrained in Postgres and compiled from `enums.js`, so translating them needs a display-mapping layer, not new strings. Deliberate, and a separate piece of work.
...
3. **F5 — payment.** The lapsed gate now says "no card required yet" and calls `/api/auth/renew` directly; that is the seam Stripe Checkout slots into without the screen changing shape (Day 10)
...
- **`toLocaleDateString(undefined, …)` is a bug, not a default.** `undefined` means the *browser's* locale, not the app's, so every such call leaks English dates to a teacher using Arabic — and it reads as correct in review. Use `useLocale()` in components, `localeFor(currentLang())` in plain functions. This is how F66 hid in the shared `DatePicker` (i.e. in every form in the product) through a whole translation sweep: key parity cannot catch it, because nothing is missing.
- **A failed fetch must become a state, not an absence.** F63: `PresentationBuilder`'s catch cleared `loading` — textbook-looking error handling — but left `row` null, and the guard read `loading || !deck`, so the loader rendered forever. F64 was the same mistake wearing the opposite mask: three builders swallowed their loads in `.catch(() => {})` and rendered a blank *editable* form for a row that wasn't there. **Use `useRowLoader()` + `LoadErrorCard` from `_shared.jsx`** — they own the 404-vs-transport split, the abort handling, and the `initialId` rule (a builder whose id lives in state re-runs its load after the first save; that pass must never blank an editor the teacher is typing in). A builder that keeps its id in state must also be **keyed** in `App.jsx`, or a change of `:id` leaves it bound to the previous row.
