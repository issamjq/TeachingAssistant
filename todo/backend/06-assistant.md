...
> **Status (2026-08-12): ✅ done end-to-end, beyond this spec** — the
> live service also emits `action` frames (`create_work`, `add_student`,
> `add_schedule_entry`, `navigate`, `set_accessibility`), and the frontend
> handles all of them: `navigate`/`set_accessibility` complete in the
> browser; the prefill actions park their payload under the
> `murchid.assistant.prefill` sessionStorage key, navigate, and the three
> target forms read it (new-student, schedule entry, studio composer, via
> `src/shared/lib/assistantPrefill.ts`). Nothing open in this file.
